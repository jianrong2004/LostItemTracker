...
class _AdminItemDetailPageState extends State<AdminItemDetailPage> {
  ...
  Future<void> _loadPossibleDuplicates() async {
    ...
    try {
      final col =
          widget.type == 'lost' ? 'lost_item_reports' : 'found_item_reports';
      final snap =
      ...
      final others =
          snap.docs
              ...
              .where((m) {
                ...
                return n.isNotEmpty &&
                    (name.isEmpty || n.contains(name) || name.contains(n));
              })
              .take(5)
              .toList();
      if (mounted) {
  ...
  Future<void> _updateItemReturnStatus(String newReturnStatus) async {
    setState(() => _loading = true);
    try {
      final col =
          widget.type == 'lost' ? 'lost_item_reports' : 'found_item_reports';
      await FirebaseFirestore.instance
          ...
          .doc(widget.reportId)
          .update({
            'itemReturnStatus': newReturnStatus,
            'updatedAt': FieldValue.serverTimestamp(),
          });
      if (mounted) {
        setState(() {
          _data['itemReturnStatus'] = newReturnStatus;
          _loading = false;
        });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (pb != null) {
      if (pb is Uint8List)
        ...
        photoBytes = Uint8List.fromList(List<int>.from(pb));
    }

    final returnStatusOptions =
        widget.type == 'lost'
            ? ['pending', 'returned']
            : ['pending', 'claimed'];

    final currentReturnStatus =
        _data['itemReturnStatus'] as String? ?? 'pending';
    final dropdownValue =
        returnStatusOptions.contains(currentReturnStatus)
            ? currentReturnStatus
            : returnStatusOptions.first;

    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.type == 'lost' ? 'Lost' : 'Found'} Item Detail'),
        backgroundColor:
            widget.type == 'lost' ? Colors.red.shade700 : Colors.green.shade700,
        foregroundColor: Colors.white,
        actions: [
          // itemReturnStatus dropdown
            ...
            child: DropdownButton<String>(
              ...
              items:
                  returnStatusOptions
                      ...
                      .toList(),
              onChanged: (v) {
                if (v != null && v != dropdownValue) _updateItemReturnStatus(v);
              },
            ),
          ),
          const SizedBox(width: 4),
          // reportStatus popup menu (submitted & draft only)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _section('Basic Info', [
                      ...
                      _row('Return status', _data['itemReturnStatus'] ?? 'N/A'),
                    ]),
                    if (widget.type == 'lost') ...[
                      _section('Lost details', [
                        _row(
                          'Lost date/time',
                          _formatDateTime(_data['lostDateTime']),
                        ),
                        _row('Address', _data['address'] ?? 'N/A'),
                        _row(
                          'Location description',
                          _data['locationDescription'] ?? 'N/A',
                        ),
                      ]),
                    ] else ...[
                      _section('Found details', [
                        _row(
                          'Found date/time',
                          _formatDateTime(_data['foundDateTime']),
                        ),
                        _row('Address', _data['address'] ?? 'N/A'),
                        _row(
                          'Drop-off desk ID',
                          _data['dropOffDeskId'] ?? 'N/A',
                        ),
                      ]),
                    ],
                    _section('Meta', [
                    ...
                    if (_duplicatesLoaded &&
                        _possibleDuplicates.isNotEmpty) ...[
                      const SizedBox(height: 16),
                      Text(
                        'Duplicate hint: similar items (same category)',
                        style: TextStyle(
                          fontSize: 14,
                    ...
                    if (widget.type == 'found') ...[
                      const SizedBox(height: 16),
                      Text(
                        'Related Claims (${_relatedClaims.length})',
                      ...
                      if (!_claimsLoaded)
                        ...
                        ..._relatedClaims.map((c) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              title: Text(
                                'Claim ${cid.length > 8 ? cid.substring(0, 8) : cid}',
                              ),
                              subtitle: Text(
                                'Status: ${c['claimStatus'] ?? 'N/A'} • ${_formatDateTime(c['createdAt'])}',
                              ),
